...
%>
        <FONT SIZE=2>
        [<A HREF="YMList.asp?<%=TArgs%>">순서형보기</A>]
        </FONT>
<%  
Else  
        TArgs = CArg1 & "&Mode=qa" 
...
%>
        <FONT SIZE=2>
        [<A HREF="YMList.asp?<%=TArgs%>">응답형보기</A>]
        </FONT>
<% 
End If 
%>       
...
' 이전 페이지 메뉴 
    If ( intPGEnd > 0 ) Then
%>
        [<A HREF="YMList.asp?<%=CArg2%>&Page=<%=intPGEnd%>">이전</A>]
<%  End If
...
%>
        [<A HREF="YMList.asp?<%=CArg2%>&Page=<%=intI%>"><%=intI%></A>] 
        &nbsp;
<%  
        intI = intI + 1
        LCount = LCount - 1
...
%>
        [<A HREF="YMList.asp?<%=CArg2%>&Page=<%=intI%>">다음</A>]
<%  End If 
%>
    </TD>
</TR>
